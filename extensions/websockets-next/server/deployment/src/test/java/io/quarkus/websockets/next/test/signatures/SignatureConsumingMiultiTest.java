...
public class SignatureConsumingMiultiTest {
    ...
    void verifyExecutionOfOnMessageWhenConsumingAndReturningMultis() {
        ...
        assertThat(client.getMessages().stream().map(Buffer::toString).collect(Collectors.toList())).containsExactlyInAnyOrderElementsOf(
                IntStream.range(0, 10).mapToObj(id -> "WS " + 3 + " received: hello" + id).collect(Collectors.toList())
        );
    }
